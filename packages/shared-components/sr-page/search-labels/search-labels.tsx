...
const SearchLabelsContent = ({ searchPayLoad }: any) => {
  ...
  useEffect(() => {
    async function getSearchLabels() {
      try {
        ...
        searchLabel = await httpClientRequest(
          displayNameBFFEndPt,
          searchPayLoad,
          "POST",
      ...
      searchLabel?.year
        ? filters.push({ key: "year", value: searchLabel?.year })
        : filters.push({ key: "year", value: "2025" });
      if (searchLabel?.studyLevel)
        filters.push({ key: "study-level", value: searchLabel?.studyLevel });
      if (searchLabel?.subjectName) {
        if (searchLabel?.subjectName?.length === 1) {
          filters.push({
            key: process.env.PROJECT === "Whatuni" ? "subject" : "course",
            value: searchLabel?.subjectName?.[0],
          });
        } else {
      ...
      if (searchLabel?.studyMode)
        filters.push({
          key: process.env.PROJECT === "Whatuni" ? "study-mode" : "study_mode",
          value: searchLabel?.studyMode,
        });
      if (searchLabel?.studyMethod)
        filters.push({
          key:
            process.env.PROJECT === "Whatuni" ? "study-method" : "study_method",
          value: searchLabel?.studyMethod,
        });
      if (searchLabel?.locationType)
        filters.push({
          key: "location-type",
          value: searchLabel?.locationType,
        });
      if (searchLabel?.universityGroup)
        filters.push({
          key: "russell-group",
          value: searchLabel?.universityGroup,
        });
      if (searchLabel?.locationName?.length === 1) {
        filters.push({
          key: searchParams?.has("region") ? "region" : "city",
          value: searchLabel?.locationName,
        });
      } else {
  ...
  const removeFilter = (filterKey: string, value: any) => {
    ...
    if (currentParams.has(filterKey)) {
      if (
        currentParams.get(filterKey)?.includes(" ") &&
        (filterKey === "subject" ||
          filterKey === "course" ||
          filterKey === "region" ||
          filterKey === "city")
      ) {
        const updatedSubjects = currentParams
          .get(filterKey)
          ?.split(" ")
          ?.filter((val) => val !== value?.toLowerCase());
        const updatedSubParam =
          updatedSubjects && updatedSubjects?.length > 0
          ...
          <ul className="flex items-start gap-[8px] uppercase">
            {filterList?.map((items, index) => (
              ...
              >
                ...
                {index !== 0 && index !== 1 ? (
                  <svg
                    onClick={() => removeFilter(items?.key, items?.value)}
