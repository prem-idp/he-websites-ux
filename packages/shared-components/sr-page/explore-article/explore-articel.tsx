...
const ExploreArticles = async ({ exploreSectionProps }: any) => {
  ...
  const richContentTitle =
    subjectComponentData?.moduleComponentsCollection?.items?.[0]
      ?.fieldComponentsCollection?.items?.[0]?.richContentTitle ||
    cityComponentData?.pageTitle;
  const richContent =
    subjectComponentData?.moduleComponentsCollection?.items?.[0]
      ?.fieldComponentsCollection?.items?.[0]?.richContent?.json?.content?.[0]
      ?.content?.[0]?.value || cityComponentData?.shortDescription;
  const imageUrl =
    subjectComponentData?.moduleComponentsCollection?.items?.[0]
      ?.fieldComponentsCollection?.items?.[0]?.homeMediaCollection?.items[0]
      ?.image?.url ||
    cityComponentData?.bannerImageCollection?.items?.[0]?.imgUpload?.url;
  return (
    <>
      <Link
      ...
      >
        <div className="flex flex-col item-center border-[1px] border-grey-200 rounded-[8px] lg:flex-row">
          ...
          <div className="p-[16px] shadow-custom-2 flex flex-col gap-[8px] text-grey300 lg:px-[24px] lg:py-[32px]">
            ...
            <div className="small line-clamp-4">{richContent}</div>
            <Link
              href={
                subjectComponentData?.pageUrl ||
                cityComponentData?.urlSlug ||
                ""
              }
