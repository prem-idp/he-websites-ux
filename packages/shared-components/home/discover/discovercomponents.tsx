...
interface DiscoverSliderInterface{
  heading: string, subheading: string, internalName: string
}

const Discovercomponents:React.FC<DiscoverSliderInterface> = async(props) => {
  
  const discovercontentfulData: any = ( await graphQlFetchFunction(discoverpodQuery(process.env.PROJECT, props.internalName)))
      .data?.contentData?.items[0]?.bodyContentCollection?.items[0]?.mediaCardsCollection?.items;
