...
describe('Reviewscomponents', () => {
  ...
  test('renders the component with heading and subheading', async () => {
    ...
    expect(screen.getByText("Honest reviews from real students")).toBeInTheDocument();
    expect(screen.getByText("Discover genuine insights from students to help you make the best choice.")).toBeInTheDocument();
  });
  ...
  test('handles empty review data', async () => {
    ...
    (getReviewDetailsFunction as jest.Mock).mockResolvedValue({ reviewDetail: [] });
    render(await Reviewscomponents(props));
