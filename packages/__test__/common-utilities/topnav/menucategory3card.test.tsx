...
import Menucategory3card from '../../shared-components/common-utilities/megamenu/menucategory3card'; // Update path if needed
...
describe('Menucategory3card', () => {
  const mockData = [
    {
      ...
      navUrl: '',
      navCtaTarget: false,
      navIcon: { url: 'https://via.placeholder.com/72' },
    },
    {
  ...
  it('renders the category title when "L2 Text" style is found in data', () => {
    render(<Menucategory3card data={mockData} />);
  ...
  it('renders the correct image with src and alt attributes', () => {
    ...
    expect(images[0]).toHaveAttribute('src', '/_next/image?url=https%3A%2F%2Fvia.placeholder.com%2F72&w=256&q=75');
    expect(images[0]).toHaveAttribute('alt', 'Megamenu thumb');
    expect(images[1]).toHaveAttribute('src', '/_next/image?url=https%3A%2F%2Fvia.placeholder.com%2F72&w=256&q=75');
    expect(images[1]).toHaveAttribute('alt', 'Megamenu thumb');
  });



  it('renders the links in a responsive grid layout', () => {
    render(<Menucategory3card data={mockData} />);

    // Check that the container has the expected grid layout classes
    const container = screen.getByRole('list');
    expect(container).toHaveClass('grid');
    expect(container).toHaveClass('grid-cols-1'); // 1 column on smaller screens
    expect(container).toHaveClass('lg:grid-cols-2'); // 2 columns on large screens
  });

  it('does not render any links if data is empty or undefined', () => {
    const { container } = render(<Menucategory3card data={[]} />);
  ...
  it('does not render any category title if "L2 Text" is not found', () => {
    ...
    render(<Menucategory3card data={noCategoryData} />);
