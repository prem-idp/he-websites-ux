...
export async function getSRMetaDetailsFromContentful(
  searchParams: any,
  pathName: string,
  params: any,
  displayNameBFFEndPt: string,
  pageType: string
) {
  ...
  const metaTitle = replaceSEOPlaceHolder(
    contentfulMetadata?.metaTile,
    metaFiltersOpted
  );
  const metaDesc = replaceSEOPlaceHolder(
    contentfulMetadata?.metaDescription,
    metaFiltersOpted
  );
  const index = getSRIndexation(
    searchParams,
    searchPayLoad,
    Number(displayNameResponse?.collegeCount)
  );
  const canonical =
    getSRCanonical(searchParams, pathName) ?? getCustomDomain() + pathName;
...
export function replaceSEOPlaceHolder(
  ...
  if (inputText?.includes("[COURSE COUNT]")) {
    inputText = inputText.replace(
      "[COURSE COUNT]",
      metaFiltersOpted?.courseCount ?? "0"
    );
  }
  if (inputText?.includes("[PROVIDER COUNT]")) {
    inputText = inputText.replace(
      "[PROVIDER COUNT]",
      metaFiltersOpted?.providerCount ?? "0"
    );
  }
...
function getSRIndexation(
  ...
  if (
    ...
    //total search result count is less than/equal to 3
...
function formSRPageURL(searchParams: any, pathName: string) {
  ...
  if (filterCount < 4 && searchParams?.qualification) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("subject=" + searchParams?.qualification);
    filterCount++;
  }
  ...
  if (filterCount < 4 && searchParams?.["study-method"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("study-method=" + searchParams?.["study-method"]);
    filterCount++;
  }
...
export function getSeoMetaFeildId(searchPayLoad: any, pageType: string) {
  ...
  const getStudylevelSeoField = (studylevel: string) => {
    if (`${process.env.PROJECT}` == "PGS") {
      if (studylevel == "M") {
        return "M";
      } else if (studylevel == "L") {
        return "L";
      } else if (studylevel == "T") {
        return "ACC-Fnd";
      } else if (studylevel == "A") {
        return "FND";
      } else if (studylevel == "N") {
        return "hnd-hnc";
      }
  ...
  if (
    ...
    const studyLevelCotentfulCode = getStudylevelSeoField(
      searchPayLoad?.parentQualification
    );
    ...
    seoMetaFeildId =
      "subject + " + `studyLevel(${studyLevelCotentfulCode}) + ` + "studyMode";
  } else if (
    ...
    seoMetaFeildId =
      "subject + " +
      `studyLevel(${studyLevelCotentfulCode}) + ` +
      "studyMode + " +
      "location";
  } else if (
    ...
    seoMetaFeildId =
      "subject + " + `studyLevel(${studyLevelCotentfulCode}) + ` + "location";
  }
