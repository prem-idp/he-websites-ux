...
export async function getSearchPageMetaDetailsFromContentful(
  searchParams: any,
  pathName: string,
  params: any
) {
  ...
  const seoMetaFeildId: string =
    process.env.PROJECT === "Whatuni"
      ? getWU_SearchSEOFieldId(searchPayLoad)
      : getPGS_SearchSEOFieldId(searchPayLoad);
  const query = getMetaDetailsQueryForSRpage(seoMetaFeildId);
  const customParams = { cache: "no-cache", next: { revalidate: 300 } };
  let contentfulMetadata = await graphQlFetchFunction(
    query,
    false,
    customParams
  );
  ...
  const metaFiltersOpted: MetaFilterTypesReplace =
    getMetaOptedDisplayNames(displayNames);

  metaTitle = replaceSEOPlaceHolder(
    contentfulMetadata?.metaTite,
    metaFiltersOpted
  );
  metaDesc = replaceSEOPlaceHolder(
    contentfulMetadata?.metaDescription,
    metaFiltersOpted
  );
  ...
  if (process.env.PROJECT == "Whatuni") {
    ...
    index = getPGS_Indexation(
      searchParams,
      searchPayLoad,
      metaFiltersOpted,
      contentfulMetadata?.robots
    );
...
export function getMetaOptedDisplayNames(
  displayNameResponse: any
): MetaFilterTypesReplace {
  return {
...
export function replaceSEOPlaceHolder(
  inputText: string,
  metaFiltersOpted: MetaFilterTypesReplace
) {
  ...
  if (inputText?.includes("[COURSE COUNT]")) {
    inputText = inputText.replace(
      "[COURSE COUNT]",
      metaFiltersOpted?.courseCount ?? ""
    );
  }
  if (inputText?.includes("[PROVIDER COUNT]")) {
    inputText = inputText.replace(
      "[PROVIDER COUNT]",
      metaFiltersOpted?.providerCount ?? ""
    );
  }
  if (inputText?.includes("[REGION]")) {
    inputText = replaceMultiplePlaceholder(
      "[REGION]",
      inputText,
      metaFiltersOpted?.location
    );
  }
  ...
  if (inputText?.includes("[SUBJECT]")) {
    inputText = replaceMultiplePlaceholder(
      "[SUBJECT]",
      inputText,
      metaFiltersOpted?.searchSubject
    );
  }
  if (inputText?.includes("[STUDY LEVEL]")) {
    inputText = inputText.replace(
      "[STUDY LEVEL]",
      metaFiltersOpted?.studylevel ?? ""
    );
  }
  if (inputText?.includes("[STUDY MODE]")) {
    inputText = inputText.replace(
      "[STUDY MODE]",
      metaFiltersOpted?.studymode ?? ""
    );
  }
...
function replaceMultiplePlaceholder(
  pattern: string,
  inputText: string,
  ...
  while (inputText?.includes(pattern)) {
    const displayText =
      selectedOptionList?.length && selectedOptionList?.length > 0
...
function getWU_Indexation(
  ...
  if (
    searchPayLoad?.searchKeyword || //keyword search
    ...
    (!searchParams?.university &&
      Number(metaFiltersOpted?.providerCount) <= 3) || //total search result count is less than/equal to 3(only SR) also for no result SR
    (searchParams?.university && Number(metaFiltersOpted?.providerCount) < 1)
  ) {
    //No result PR
...
function getWU_Canonical(
  ...
  if (multiOptionSelected && !searchParams?.university)
    canonicalUrl = formSRPageURL(
      {},
      qualInUrl && qualInUrl != "" ? `/${qualInUrl}` : pathName
    );
...
function getPGS_Indexation(
  searchParams: any,
  searchPayLoad: any,
  metaFiltersOpted: MetaFilterTypesReplace,
  contentfulIndex: string
) {
  let multiOptionSelected: boolean = false;
  let filterCount = 0;
  ...
  if (
    ...
    searchPayLoad?.intakeYear ||
    searchPayLoad?.intakeMonth
  ) {
    //started date is selected
...
function getPGS_Canonical(
  ...
  const { sort, pageno, ...newObj } = { ...searchParams };
  let canonicalUrl,
    multiOptionSelected: boolean = false;
  Object.keys(searchParams).forEach((key: string) => {
    if (getselectedCount(searchParams[key]) >= 2) {
      multiOptionSelected = true;
    }
...
function formSRPageURL(searchParams: any, pathName: string) {
  ...
  if (filterCount < 4 && searchParams?.university) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 4 && searchParams?.subject) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("subject=" + formatMultiSelctedOption(searchParams?.subject));
    filterCount = filterCount + getselectedCount(searchParams?.subject);
  }
  if (filterCount < 4 && searchParams?.course) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("course=" + formatMultiSelctedOption(searchParams?.course));
    filterCount = filterCount + getselectedCount(searchParams?.course);
  }
  if (filterCount < 4 && searchParams?.q) {
    formURL =
      formURL + (formURL.includes("?") ? "&" : "?") + ("q=" + searchParams?.q);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.keyword) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 4 && searchParams?.qualification) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 4 && searchParams?.location) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("location=" + formatMultiSelctedOption(searchParams?.location));
    filterCount = filterCount + getselectedCount(searchParams?.location);
  }
  if (filterCount < 4 && searchParams?.["study_mode"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("study_mode=" + searchParams?.["study_mode"]);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.["study-mode"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("study-mode=" + searchParams?.["study-mode"]);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.["intake-year"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("intake-year=" + searchParams?.["intake-year"]);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.["intake-month"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("intake-month=" + searchParams?.["intake-month"]);
    filterCount =
      filterCount + getselectedCount(searchParams?.["intake-month"]);
  }
  if (filterCount < 4 && searchParams?.distance) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 4 && searchParams?.universityGroup) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("universityGroup=" + searchParams?.universityGroup);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.score) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 4 && searchParams?.["location-type"]) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("location-type=" + searchParams?.["location-type"]);
    filterCount++;
  }
  if (filterCount < 4 && searchParams?.sort) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
  ...
  if (filterCount < 5 && searchParams?.pageno) {
    formURL =
      formURL +
      (formURL.includes("?") ? "&" : "?") +
      ("pageno=" + searchParams?.pageno);
    filterCount++;
  } //if pageno applied then atmost 5 query params can contain
...
export function getWU_SearchSEOFieldId(searchPayLoad: any) {
  const locationSelected =
    searchPayLoad?.location?.length <= 0
      ? false
      : searchPayLoad?.location?.length >= 1 &&
          searchPayLoad?.location?.[0] != ""
        ? true
        : false;
  const subjectSelected =
    searchPayLoad?.searchSubject?.length <= 0
      ? false
      : searchPayLoad?.searchSubject?.length >= 1 &&
          searchPayLoad?.searchSubject?.[0] != ""
        ? true
        : false;
  const keywordSelected =
    searchPayLoad?.searchKeyword && searchPayLoad?.searchKeyword?.trim() != ""
      ? true
      : false;
  const universitySelected =
    searchPayLoad?.university && searchPayLoad?.university?.trim() != ""
      ? true
      : false;
  ...
  if (!universitySelected) {
    ...
    if (
      ...
      // only studyLevel(for each study levels diff text possible)
      !(subjectSelected || keywordSelected) &&
      !locationSelected &&
      searchPayLoad?.parentQualification &&
      !searchPayLoad?.studyMode
      ...
      // subject + studyLevel(for each study levels diff text possible)
      ((subjectSelected && searchPayLoad?.searchSubject?.length == 1) ||
        keywordSelected) &&
      !locationSelected &&
      searchPayLoad?.parentQualification &&
...
export function getPGS_SearchSEOFieldId(searchPayLoad: any) {
  ...
  const startDateSelected =
    searchPayLoad?.intakeYear &&
    searchPayLoad?.intakeYear?.trim() != "" &&
    searchPayLoad?.intakeMonth?.trim() != ""
      ? true
      : false;
  let seoMetaFeildId = "Default";
  ...
  if (!universitySelected) {
    ...
    if (
      ...
      // subject + qualification + location
      (subjectSelected || keywordSelected) &&
      locationSelected &&
      searchPayLoad?.childQualification &&
      !searchPayLoad?.studyMethod &&
      ...
      // subject + startDate + location + studyMethod
      (subjectSelected || keywordSelected) &&
      locationSelected &&
      !searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
      ...
      // qualification + studyMethod + location
      !(subjectSelected || keywordSelected) &&
      locationSelected &&
      searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
      ...
      // qualification + studyMethod + startDate
      !(subjectSelected || keywordSelected) &&
      !locationSelected &&
      searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
      ...
      // qualification + studyMethod + startDate + location
      !(subjectSelected || keywordSelected) &&
      locationSelected &&
      searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
    ...
    if (
      ...
      // university + studyMethod + qualification
      !(subjectSelected || keywordSelected) &&
      !locationSelected &&
      searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
      ...
      // university + studyMethod + startDate
      !(subjectSelected || keywordSelected) &&
      !locationSelected &&
      !searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
      ...
      // subject + university + studyMethod
      (subjectSelected || keywordSelected) &&
      !locationSelected &&
      !searchPayLoad?.childQualification &&
      searchPayLoad?.studyMethod &&
...
export function form_PGS_SR_breadcrumb(
  searchSEOPayload: any,
  displayNames: any,
  pathName: string
) {
  const qualSelected =
    searchSEOPayload?.childQualification &&
    searchSEOPayload?.childQualification?.trim() !== "";
  const uniSelected =
    searchSEOPayload?.university && searchSEOPayload?.university?.trim() !== "";
  const subjectSelected =
    searchSEOPayload?.searchSubject?.length <= 0
      ? false
      : searchSEOPayload?.searchSubject?.length >= 1 &&
          searchSEOPayload?.searchSubject?.[0] != ""
        ? true
        : false;
  const parentSubjectSelected =
    displayNames?.parentSubjectTextKey?.length <= 0
      ? false
      : displayNames?.parentSubjectTextKey?.length >= 1 &&
          displayNames?.parentSubjectTextKey?.[0] != ""
        ? true
        : false;
  const keywordSelected =
    searchSEOPayload?.searchKeyword &&
    searchSEOPayload?.searchKeyword?.trim() != ""
      ? true
      : false;
  const locationSelectd =
    searchSEOPayload?.location?.length <= 0
      ? false
      : searchSEOPayload?.location?.length >= 1 &&
          searchSEOPayload?.location?.[0] != ""
        ? true
        : false;
  ...
  const urlSubject =
    searchSEOPayload?.searchSubject &&
    searchSEOPayload?.searchSubject?.length > 0
      ? searchSEOPayload?.searchSubject[0]
      : "";
  ...
  const urlLocation =
    searchSEOPayload?.location && searchSEOPayload?.location?.length > 0
      ? searchSEOPayload?.location[0]
      : undefined;
  ...
  if (qualSelected) {
    ...
    qualBC = [
      {
        url: `/${searchSEOPayload?.childQualification}`,
        label: displayNames?.studyLevel ?? searchSEOPayload?.childQualification,
      },
      {
        url: formSRPageURL(urlObj, pathName),
        label: `${displayNames?.studyLevel ?? searchSEOPayload?.childQualification} Search Results`,
      },
    ];
  }
  if (subjectSelected || keywordSelected) {
    //subject
    const urlObj = {
      course: urlSubject || urlKeyword,
      qualification: searchSEOPayload?.childQualification,
    };
    subBC = displaySubject
      ? [{ url: formSRPageURL(urlObj, pathName), label: displaySubject }]
      : [];
    if (
      parentSubjectSelected &&
      displayNames?.parentSubjectName?.length > 0 &&
      urlParentSubject?.trim() != urlSubject?.trim()
    ) {
      const parentUrlObj = {
      ...
      subBC =
        urlParentSubject && displayParentSubject
          ? [
              {
                url: formSRPageURL(parentUrlObj, pathName),
  ...
  if (uniSelected) {
    ...
    uniBC = displayUniversity
      ? [
          {
            url: formSRPageURL(urlObj, pathName),
            label: displayUniversity ?? urlUniversity,
          },
        ]
      : [];
  }
  ...
  if (locationSelectd) {
    ...
    locBC = displayLocation
      ? [{ url: formSRPageURL(urlObj, pathName), label: displayLocation }]
      : [];
  }
  if (searchSEOPayload?.studyMethod) {
    //studyMode
    const urlObj = {
      course: urlSubject || urlKeyword,
      location: urlLocation,
    ...
    studyModeBC = displayStudyMethod
      ? [
          {
            url: formSRPageURL(urlObj, pathName),
            label: displayStudyMethod ?? urlStudymethod,
          },
        ]
      : [];
  }
...
export function get_WU_SR_breadcrumb(
  searchSEOPayload: any,
  displayNameResponse: any,
  qualInUrl: string
) {
  const subjectDisplayName =
    displayNameResponse?.subjectName?.length >= 0
      ? displayNameResponse?.subjectName[0]
      : "";
  const get_find_a_course_url_label = (qualCode: string) => {
    switch (qualCode) {
      case "M":
        return ["/degrees/courses/", "Courses"];
      case "L":
        return ["/postgraduate-courses/", "Postgraduate"];
      case "A":
        return ["/foundation-degree-courses/", "Foundation Degree"];
      case "T":
        return ["/access-foundation-courses/", "Access Foundation"];
      case "N":
        return ["/hnd-hnc-courses/", "HND/HNC"];
      default:
        return ["/degrees/courses/", "Courses"];
    }
  };
  const [qualUrl, qualLabel] = get_find_a_course_url_label(
    searchSEOPayload?.parentQualification
  );
  const breadcrumb_courses = searchSEOPayload?.parentQualification
    ? [{ url: qualUrl, label: qualLabel }]
    : [];
  const breadCrumb_subject =
    searchSEOPayload?.searchSubject &&
    searchSEOPayload?.searchSubject?.length >= 1
      ? [
          {
            url: `/${qualInUrl}/search?subject=${searchSEOPayload?.searchSubject?.[0]}`,
            label: `${subjectDisplayName} courses`,
          },
        ]
      : [];
  const breadCrumb_keyword = searchSEOPayload?.searchKeyword
    ? [
        {
          url: `/${qualInUrl}/searcch?q=${searchSEOPayload?.searchKeyword}`,
          label: `${subjectDisplayName} Courses`,
        },
      ]
    : [];
